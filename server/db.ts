...
export async function getAvailableDrivers(vehicleType: string) {
  const db = await getDb();
  if (!db) return [];
  ...
  return result.filter((d) => d.vehicleType === vehicleType);
}

// Delivery Partner queries
export async function getDeliveryPartnerByUserId(userId: number) {
  const db = await getDb();
  if (!db) return undefined;
...
export async function getUserRides(userId: number) {
  const db = await getDb();
  if (!db) return [];
...
export async function getDeliveryOrderById(orderId: number) {
  const db = await getDb();
  if (!db) return undefined;
...
export async function getUserDeliveryOrders(userId: number) {
  ...
  return await db
    .select()
    .from(deliveryOrders)
    .where(eq(deliveryOrders.customerId, userId));
}

export async function getPartnerDeliveryOrders(partnerId: number) {
  const db = await getDb();
  if (!db) return [];
  ...
  return await db
    .select()
    .from(deliveryOrders)
    .where(eq(deliveryOrders.deliveryPartnerId, partnerId));
}
...
export async function getUserCart(userId: number) {
  ...
  return await db
    .select()
    .from(cartItems)
    .where(eq(cartItems.customerId, userId));
}
